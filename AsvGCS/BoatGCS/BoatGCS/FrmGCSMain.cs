...
namespace BoatGCS
{
    public partial class FrmGCSMain : Form
    {
        ...
        public const byte MIX_BOTH_CONTROL = 2;
                        ...
                        for (i = 0; i < (rcvQTail - 7); i++) //rcvQTail-7:除去头部ID的5个字节和长度的2个字节
                        {
                               ...
                               )//包头正确
                            {
                                ...
                                if ((rcvQTail - i) >= pack_len)//缓冲区内数据长度超过包长度
                                {
                                    ...
                                    //先做判断是否是定位信息，根据北斗设备的地址是否是  204618--定位的北斗模块
                                    //208578--发送实时数据的北斗模块

                                    if (recvpackbuf[11]*Math.Pow(2,16)+recvpackbuf[12]*Math.Pow(2,8)+recvpackbuf[13]==204618)
                                    {
                                        //recvpackbuf[21]是精度的度开始字节
                                        int beidou_longitude_degree;
                                        int beidou_longitude_minute;

                                        int beidou_longitude_second_high;
                                        int beidou_longitude_second_low;
                                        int beidou_longitude_second_deci;
                                        double beidou_longitude_second;

                                        double beidou_longitude;

                                        beidou_longitude_degree = recvpackbuf[21+1];

                                        beidou_longitude_minute = recvpackbuf[22 + 1];
                                        beidou_longitude_minute = (beidou_longitude_minute & 0xfc)/4;

                                        beidou_longitude_second_high = recvpackbuf[22 + 1];
                                        beidou_longitude_second_high = beidou_longitude_second_high & 0x03;
                                        beidou_longitude_second_low = recvpackbuf[23 + 1];
                                        beidou_longitude_second_low = (beidou_longitude_second_low & 0xf0)/16;
                                        beidou_longitude_second_deci = recvpackbuf[23 + 1];
                                        beidou_longitude_second_deci = beidou_longitude_second_deci & 0x0f;

                                        beidou_longitude_second = beidou_longitude_second_high * 16 + beidou_longitude_second_low + Convert.ToDouble(beidou_longitude_second_deci) * 0.1;

                                        beidou_longitude = Convert.ToDouble(beidou_longitude_degree) + Convert.ToDouble(beidou_longitude_minute) / 60 + beidou_longitude_second/3600;


                                        int beidou_lattitude_degree;
                                        int beidou_lattitude_minute_high;
                                        int beidou_lattitude_minute_low;
                                        int beidou_lattitude_minute;

                                        int beidou_lattitude_second_high;
                                        int beidou_lattitude_second_low;
                                        int beidou_lattitude_second_integer;
                                        int beidou_lattitude_second_deci;
                                        double beidou_lattitude_second;

                                        double beidou_lattitude;

                                        beidou_lattitude_degree = recvpackbuf[24 + 1];
                                        beidou_lattitude_degree = (beidou_lattitude_degree & 0xfc) / 4;

                                        beidou_lattitude_minute_high = recvpackbuf[24 + 1];
                                        beidou_lattitude_minute_high = beidou_lattitude_minute_high & 0x03;

                                        beidou_lattitude_minute_low = recvpackbuf[25 + 1];
                                        beidou_lattitude_minute_low = (beidou_lattitude_minute_low & 0xf0) / 16;

                                        beidou_lattitude_minute = beidou_lattitude_minute_high * 16 + beidou_lattitude_minute_low;

                                        beidou_lattitude_second_high = recvpackbuf[25 + 1];
                                        beidou_lattitude_second_high = beidou_lattitude_second_high & 0x0f;
                                        beidou_lattitude_second_low = recvpackbuf[26 + 1];
                                        beidou_lattitude_second_low = (beidou_lattitude_second_low & 0xc0)/64;
                                        beidou_lattitude_second_integer=beidou_lattitude_second_high*4+beidou_lattitude_second_low;
                                        beidou_lattitude_second_deci = recvpackbuf[26 + 1];
                                        beidou_lattitude_second_deci = (beidou_lattitude_second_deci & 0x3c)/4;


                                        beidou_lattitude_second = Convert.ToDouble(beidou_lattitude_second_integer) + Convert.ToDouble(beidou_lattitude_second_deci) / 10;
                                        beidou_lattitude = Convert.ToDouble(beidou_longitude_degree) + Convert.ToDouble(beidou_lattitude_minute)/60 + beidou_lattitude_second/3600;

                                        textBox_beidou_longitude.Text = Convert.ToString(beidou_longitude);
                                        textBox_beidou_lattitude.Text = Convert.ToString(beidou_lattitude);


                                    }

                                    if (recvpackbuf[11] * Math.Pow(2, 16) + recvpackbuf[12] * Math.Pow(2, 8) + recvpackbuf[13] == 208578)
                                    {
                                        checksum = 0;
...
*/
            ...
            if (gbl_var.ap2gcs_real_cnt != gbl_var.ap2gcs_real_cnt_lst)
            {
                ...
                /*实时数据显示在在控制菜单中，这样就不用来回切换监视菜单和控制菜单了*/
                teBox_fb_oo_pwm.Text = Convert.ToString(ap2gcs_real.moo_pwm);
                //teBox_fb_bf_pwm.Text = Convert.ToString(ap2gcs_real.mbf_pwm);
                teBox_fb_yaw_pwm.Text = Convert.ToString(ap2gcs_real.rud_pwm);
                textBox_feedbck_speed.Text = Convert.ToString(ap2gcs_real.spd * 0.01 * 1.944);
        ...
        private void button_mix_both_control_Click(object sender, EventArgs e)
        {
            /*这个是所有的参数设置都要把这个置为true*/
            gbl_var.send_parameter_set = true;
